...
def create_features(url):
    result = []
    result.append(contains_ip_address(url))
    result.append(days_since_registration(url))
    result.append(check_ssl_cert(url))
    result.append(get_redirect_count(url))
    result.append(check_indexed_in_google(url))
    result.append(check_domain_indexed_in_google(url))
    result.append(is_url_shortened(url))
    return np.reshape(result, (1, len(result)))
